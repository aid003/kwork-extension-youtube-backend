...
/**
 * Генерирует временные метки (timestamps) по транскрипту видео.
 * • language — язык описаний (en / ru …)
 * • detail   — уровень детализации описания каждой метки
...
export async function generateTimestamps(
  ...
  const systemPrompt =
    prompts.timestamps +
    `\n\nЯзык ответа: ${language}\nУровень детализации описаний: ${detail}`;

  /* ---------- logging (request) ---------- */
  console.log("───────────────────────────────────────");
  console.log("[Gemini] generateTimestamps()");
